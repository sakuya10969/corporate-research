  ...
  return (
    <Menu shadow="md" width={160}>
      ...
      <Menu.Dropdown>
        ...
        <Menu.Item onClick={() => handleDownload("docx")}>
          Word (.docx)
        </Menu.Item>
      </Menu.Dropdown>
    </Menu>
...
function HistoryTab({ companyId }: { companyId: string }) {
  const { data, isLoading } =
    useGetCompanyRunsApiCompaniesCompanyIdRunsGet(companyId);
        ...
        >
          <Stack gap={2}>
            <Group gap="xs">
              ...
              <Badge size="xs" color={statusColor[run.status] ?? "gray"}>
                {run.status}
              </Badge>
...
function ScoreCard({
  ...
  scores: NonNullable<AnalysisResponse["scores"]>;
}) {
  const items = [
          ...
          return (
            <Group key={key} justify="space-between">
              ...
              <Group gap="xs">
                ...
                <Text size="sm" fw={600} c="#1E293B" w={36}>
                  {item.score}
                </Text>
              </Group>
              {item.reason && (
                <Text size="xs" c="#64748B" style={{ flex: 1 }}>
                  {item.reason}
                </Text>
              )}
            </Group>
        ...
        <Stack gap="md">
          ...
          {profile &&
            (profile.name ||
              ...
              <CompanyCard title="企業プロフィール">
                <Stack gap={4}>
                  {profile.name && (
                    <ProfileRow label="社名" value={profile.name} />
                  )}
                  {profile.founded && (
                    <ProfileRow label="設立" value={profile.founded} />
                  )}
                  {profile.ceo && (
                    <ProfileRow label="代表者" value={profile.ceo} />
                  )}
                  {profile.location && (
              ...
              <CompanyCard title="事業領域">
                <Group gap="xs">
                  {structured.business_domains.map((d) => (
                    <Badge key={d} variant="light" color="blue" size="md">
                      {d}
                    </Badge>
                  ))}
                </Group>
          ...
          {summary?.swot && (
            <CompanyCard title="SWOT分析">
              <Stack gap="sm">
                {(summary.swot.strengths ?? []).length > 0 && (
                  <SwotSection
                    label="強み (Strengths)"
                    items={summary.swot.strengths ?? []}
                  />
                )}
                {(summary.swot.weaknesses ?? []).length > 0 && (
                  <SwotSection
                    label="弱み (Weaknesses)"
            ...
            <CompanyCard title="競合企業（推定）">
              <Group gap="xs">
                {summary.competitors.map((c) => (
                  <Badge key={c} variant="light" color="gray" size="md">
                    {c}
                  </Badge>
                ))}
              </Group>
